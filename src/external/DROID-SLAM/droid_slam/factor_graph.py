...
class FactorGraph:
    ...
    def rm_keyframe(self, ix):
        ...
        if hasattr(self.video, "dynmask"):
            self.video.dynmask[ix : t - 1] = self.video.dynmask[ix + 1 : t].clone()
    ...
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False):
        ...
        with autocast(enabled=False):
            ...
            if hasattr(self.video, "dynmask"):
                dyn_i = self.video.dynmask[self.ii].to(self.device)              # [E, ht, wd]
                static_i = (dyn_i == 0).float()[None, :, :, :, None]             # [1, E, ht, wd, 1]
                self.weight *= static_i
        ...
        for step in range(steps):
            ...
            with CudaTimer("backend", enabled=False):
                ...
                for i in range(self.ii.min(), self.jj.max()+1, s):
                    ...
                    if hasattr(self.video, "dynmask"):
                        dyn_i = self.video.dynmask[iis].to(self.device)                  # [Ev, ht, wd]
                        static_i = (dyn_i == 0).float()[None, :, :, :, None]
                        self.weight[:, v] *= static_i
