...
class DepthVideo:
    def __init__(self, image_size=[480, 640], buffer=1024, stereo=False, device="cuda:0"):
        ...
        self.dynmask = torch.zeros(buffer, ht//8, wd//8, device=device, dtype=torch.uint8).share_memory_()
    ...
    def to(self, device="cuda"):
        ...
        self.dynmask = self.dynmask.to(device=device)
    ...
    def __item_setter(self, index, item):
        ...
        if len(item) > 9 and item[9] is not None:
            ...
            m = (m > 0).to(torch.float32).to(device=self.disps.device)  # binary float

            # downsample to (ht//8, wd//8) if needed
            if m.shape[-2:] != (self.ht//8, self.wd//8):
                m = F.interpolate(m[None, None], size=(self.ht//8, self.wd//8), mode="nearest")[0, 0]

            self.dynmask[index] = m.to(torch.uint8)  # 1=dynamic
