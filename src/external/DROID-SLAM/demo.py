...
def image_stream(imagedir, calib, stride, maskdir=None):
    ...
    for t, imfile in enumerate(image_list):
        ...
        if maskdir is not None:
            stem, ext = os.path.splitext(imfile)
            mpath = os.path.join(maskdir, stem + ext)
            if os.path.isfile(mpath):
                dynmask = cv2.imread(mpath, cv2.IMREAD_GRAYSCALE)  # 0..255
        if len(calib) > 4:
            image = cv2.undistort(image, K, calib[4:])
            if dynmask is not None:
                dynmask = cv2.undistort(dynmask, K, calib[4:])  # will get gray; we binarize later
        ...
        yield t, image[None], intrinsics, dynmask
...
if __name__ == '__main__':
    ...
    parser.add_argument("--maskdir", type=str, default=None, help="path to dynamic mask directory (same filenames as images)")
    ...
    tstamps = []
    for (t, image, intrinsics, dynmask) in tqdm( image_stream(args.imagedir, args.calib, args.stride, args.maskdir)):
        if t < args.t0:
            continue
    ...
    traj_est = droid.terminate(image_stream(args.imagedir, args.calib, args.stride, args.maskdir))
