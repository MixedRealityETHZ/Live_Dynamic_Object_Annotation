...
using System.Text;                       // ADDED for UTF8 JSON
...
public class PassthroughQuadAlignerWithSegmentation : MonoBehaviour
{
    ...
    // ADDED — send serialization and state
    private readonly SemaphoreSlim sendLock = new SemaphoreSlim(1, 1);
    private int frameId = 0;
    private bool wasPinchingL, wasPinchingR;

    // ADDED — JSON message shape
    [Serializable]
    private class ClickMsg
    {
        public string cmd = "click";
        public int frame;
        public float u;     // 0..1 (x)
        public float v;     // 0..1 (y; top-left origin; see SendClickJson)
        public int texW;
        public int texH;
    ...
    private void Start()
    {
        ...
        // ADDED — warn if MeshCollider missing (needed for textureCoord UVs)
    ...
    private IEnumerator CaptureAndSendLoop()
    {
        ...
        while (true)
        {
            ...
            // ADDED — bump frameId to correlate with click JSON
            frameId++;

            var jpg = encodeTex.EncodeToJPG(jpgQuality);

            // ADDED — serialize all sends (no concurrent SendAsync on ClientWebSocket)
    ...
    private async void SendClickJson(Vector2 uv, Vector3 world, string whichHand, int texW, int texH)
    {
        ...
        var msg = new ClickMsg
        {
            frame = frameId,
            u = uv.x,
            // Unity UV origin is bottom-left; flip V so (0,0) is top-left for server-side CV convenience.
            v = 1f - uv.y,
            texW = texW,
            texH = texH,
            worldX = world.x,
    ...
    // ADDED — single-file send serializer for both JPEG and JSON
...
/*        public int frame;
    ...
    private void Start()
    {
        ...
        var r = cameraQuad.GetComponent<Renderer>();
        ...
        var mat = r.material; // instance; we’ll clean it up on destroy
